...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (d.action === "register") {
      ...
      if (ex.rows.length > 0) return err("Account already exists with this email");
      const id = "er-" + Date.now();
      await db.execute({
        sql: "INSERT INTO employers (id,company_name,contact_person,email,phone,industry,password_hash,created_at) VALUES (?,?,?,?,?,?,?,?)",
        args: [id, d.companyName, d.contactPerson, d.email, d.phone||"", d.industry||"", hash(d.password), new Date().toISOString()]
      });
